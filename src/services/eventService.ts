...
export const createEvent = async (eventData: EventCreate, userId: number, imageFile?: Express.Multer.File) => {
    ...
    try {
        ...
        if (eventData.tags && eventData.tags.length > 0) {
            const tagPromises = eventData.tags.map(tag => 
        ...
        if (eventData.dates && eventData.dates.length > 0) {
            const datePromises = eventData.dates.map(date =>
...
export const addAttendee = async (userId: number, eventId: string) => {
  ...
  export const removeAttendee = async (userId: number, eventId: string) => {
